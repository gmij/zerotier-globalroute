...
process_template() {
    ...
    if [ -n "$CONFIG_FILE" ] && [ -f "$CONFIG_FILE" ]; then
        ...
        # 特殊处理包含空格的变量 - 使用 awk 更安全
        awk -v tcp_rmem="${TCP_RMEM:-4096 87380 16777216}" \
            -v tcp_wmem="${TCP_WMEM:-4096 65536 16777216}" \
            '{
                gsub(/TCP_RMEM/, tcp_rmem)
                gsub(/TCP_WMEM/, tcp_wmem)
                print
            }' "$temp_file" > "${temp_file}.tmp" && mv "${temp_file}.tmp" "$temp_file"
    fi
