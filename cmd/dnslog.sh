...
# DNS日志配置变量（优先使用配置文件中的值）
DNS_LOG_FILE="${DNS_LOG_FILE:-$SCRIPT_DIR/logs/zt-dns-queries.log}"
DNSMASQ_LOG_FILE="${DNSMASQ_LOG_FILE:-$SCRIPT_DIR/logs/dnsmasq.log}"
DNS_LOG_MAX_SIZE="${DNS_LOG_MAX_SIZE:-10485760}"  # 10MB
DNS_LOG_MAX_DAYS="${DNS_LOG_MAX_DAYS:-7}"  # 保留7天日志
...
setup_dns_log_rotation() {
    ...
    if ! crontab -l 2>/dev/null | grep -q "zt-dns-queries"; then
        # 每天轮转日志文件并删除旧日志
        local temp_cron=$(mktemp)
        crontab -l 2>/dev/null > "$temp_cron"

        cat >> "$temp_cron" << EOF
# ZeroTier Gateway DNS日志轮转
0 0 * * * if [ -f "$DNS_LOG_FILE" ] && [ \$(stat -c%s "$DNS_LOG_FILE") -gt $DNS_LOG_MAX_SIZE ]; then mv "$DNS_LOG_FILE" "$DNS_LOG_FILE.\$(date +\\%Y\\%m\\%d)"; fi
0 0 * * * if [ -f "$DNSMASQ_LOG_FILE" ] && [ \$(stat -c%s "$DNSMASQ_LOG_FILE") -gt $DNS_LOG_MAX_SIZE ]; then mv "$DNSMASQ_LOG_FILE" "$DNSMASQ_LOG_FILE.\$(date +\\%Y\\%m\\%d)"; fi
10 0 * * * /bin/find $(dirname "$DNS_LOG_FILE")/ -name 'zt-dns-queries.log.*' -mtime +$DNS_LOG_MAX_DAYS -delete
10 0 * * * /bin/find $(dirname "$DNSMASQ_LOG_FILE")/ -name 'dnsmasq.log.*' -mtime +$DNS_LOG_MAX_DAYS -delete
EOF
